...
type MockMinMaxIndexReader struct {
}

func (m MockMinMaxIndexReader) MayBeInFragment(fragId uint32) (bool, error) {
	if fragId == uint32(0) {
		return true, nil
	}
	return false, nil
...
func (m MockMinMaxIndexReader) GetFragmentRowCount(fragId uint32) (int64, error) {
	return 0, nil
}

func (m MockMinMaxIndexReader) ReInit(file interface{}) error {
	return nil
}

func (m MockMinMaxIndexReader) StartSpan(span *tracing.Span) {
}

func (m MockMinMaxIndexReader) Close() error {
	return nil
}
...
func TestSKIndexScanUsingMinMarksForSeek(t *testing.T) {
	reader := sparseindex.NewSKIndexReader(2, 2, 1)
	option := &query.ProcessorOptions{Condition: &influxql.BinaryExpr{
		Op:  influxql.EQ,
		LHS: &influxql.VarRef{Val: "value", Type: influxql.Integer},
		RHS: &influxql.IntegerLiteral{Val: 2},
	...
	r := &MockMinMaxIndexReader{}

	var frs = fragment.FragmentRanges{{Start: 0, End: 4}}
	frs, _, err = reader.Scan(r, frs)
	require.NoError(t, err)
	require.Equal(t, 1, len(frs))
