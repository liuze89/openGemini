...
func newIndexCache(path string, store bool) *IndexCache {
	...
	ic := &IndexCache{
		tagFilterCache: workingsetcache.New(tagCacheSize, cacheExpireDuration),
		path:           path,
	}
	if store {
		...
		ic.SeriesKeyToTSIDCache = workingsetcache.New(tsidCacheSize, cacheExpireDuration)
		ic.TSIDToSeriesKeyCache = workingsetcache.New(skeyCacheSize, cacheExpireDuration)
		ic.TagKeyValueCache = workingsetcache.New(skeyCacheSize, cacheExpireDuration)
	}
	ic.TagFilterCostCache = workingsetcache.New(tagFilterCostSize, cacheExpireDuration)
	ic.store = store
