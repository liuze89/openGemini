...
func (s *Backup) FullBackupTableFile(sh Shard, t immutable.TablesStore, peersPtIDMap map[uint32]*NodeInfo, name string, isOrder bool, nodePath, outPath string) ([][]string, error) {
	...
	for _, f := range orderfiles {
		...
		if err := copyTableFile(f, nil, sh, peersPtIDMap, nodePath, outPath, &fileList); err != nil {
			return fileList, err
		}
	}
	if len(csFiles) > 0 {
		...
		if err := backup.FileCopy(countPath, filepath.Join(outPath, countPath)); err != nil {
...
func copyTableFile(f immutable.TSSPFile, seen map[string]bool, sh Shard, peersPtIDMap map[uint32]*NodeInfo, nodePath, outPath string, fileList *[][]string) error {
	f.RefFileReader()
	defer func() {
		f.UnrefFileReader()
	}()
	...
	*fileList = append(*fileList, fileListItem)
	...
	if f.GetPkInfo() != nil {
		...
		*fileList = append(*fileList, []string{idxPath})
		if err := backup.FileCopy(idxPath, filepath.Join(outPath, idxPath)); err != nil {
	...
	if f.GetSkipIndexInfo() != nil {
		for _, s := range f.GetSkipIndexInfo() {
			*fileList = append(*fileList, []string{s.Name()})
			if err := backup.FileCopy(s.Name(), filepath.Join(outPath, s.Name())); err != nil {
				log.Error("backup colstore index file error", zap.Error(err))
				return err
			}
		}
...
func (s *Backup) IncBackupTableFile(sh Shard, t immutable.TablesStore, peersPtIDMap map[uint32]*NodeInfo, name string, isOrder bool, nodePath, outPath string) ([][]string, [][]string, error) {
	...
	for _, f := range orderfiles {
		...
		if err := copyTableFile(f, seen, sh, peersPtIDMap, nodePath, outPath, &addFileList); err != nil {
