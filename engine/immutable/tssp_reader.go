...
func (f *tsspFile) Close() error {
	...
	util.WaitTimeOut(f.wg.Wait, f.ForceDone, 10*time.Minute)
...
func (f *tsspFile) SetPkInfo(pkInfo *colstore.PKInfo) {
	...
	switch f.pkInfo.GetPkType() {
	case meta.PrimaryKeyTypeCluster:
		pkRecord := pkInfo.GetRec()
		ci := sparseindex.NewClusterIndex()
		err := ci.WritePkRec(pkRecord)
		if err != nil {
			// The clustered index construction failed; forcibly changing the file index type.
			f.pkInfo.SetPkType(meta.PrimaryKeyTypeNormal)
