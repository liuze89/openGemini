...
- 全局字典，在编码Record, ChunkMeta 等信息时，通过字典将 field 名称替换为字典编号
- 可以提升数据压缩率和数据解码效率
- 在新增字典时，只能追加文件，不能修改已有字典文件；并保持字典文件顺序不变
...
- Level Compaction任务的最大并发数。0表示采用默认值。
...
- 类型: `int`
- 默认值: `1`
- Full Compaction任务的最大并发数。介于于 1-32之间。
...
- Type: `int`
- Default: `1`
- Compaction 模式. 0 自动选择; 1 stream 模式 内存消耗更小; 2 普通模式

**correct-time-disorder**
- Type: `bool`
- Default: `false`
- 在数据文件由于各种原因，文件之间存在时间交叉、顺序错误等情况下，开启该配置，将在compaction过程自动修复（若没有其他BUG）。配置为true时，compaction-method 强制为 2

**max-compaction-level**
- Type: `int`
- Default: `0`
- 设置level compact的层级上限，默认0表示不限制
...
- 类型: `bool`
- 默认值: `false`
- 配置为 true 将开启hot 模式，最近写入的数据将被全量缓存到内存
...
- 类型: `toml.Duration`
- 默认值: `60s`
- hot 数据分区，当前缓存达到上限，或数据过期时，hot数据将转变为 warm 数据
- time window根据数据文件最大时间进行计算
- 数据过期时，选择最老的时间窗，将时间窗内的数据文件从hot 转变为 warm
...
- default: `2`
- 数据写入可靠性配置
- 1: 低可靠性. 内核进程故障可能导致数据丢失
- 2: 中可靠性. 容器或虚拟机故障时可能导致数据丢失. (default)
- 3: 高可靠性. 磁盘等存储介质故障时可能导致数据丢失

**max-wal-file-size = "256m"**

- 类型: `toml.Size`
- 默认值: `256m`
- 单个 WAL 文件在磁盘上的最大大小，超过该值后，切换新wal

**max-wal-duration = "300s"**

- 类型: `toml.Duration`
- 默认值: `300s`
- 单个 WAL 保持时间，超过该配置值，即使没达到 `max-wal-file-size` 配置的值，也强行切换新wal

**wal-compress-mode = 1**

- 类型: `int`
- 默认值: `1`
- WAL 数据压缩算法。 0 不压缩； 1 LZ4；2 Snappy

**concurrent = 0**

- 类型: `int`
- 默认值: `0`
- 写入并发数，默认值为: CPU核心数量/2

**series-hash-factor = 1**

- 类型: `int`
- 默认值: `1`
- 默认情况下，按表名称的HASH值进行分组，如果该配置项的值大于1，将按照时间线的HASH值进行二次分组
...
- 类型: `bool`
- 默认值: `false`
- 参考shelf模式相关配置和说明，设置为true
- 这可能是一个临时配置，由于内核尚未完全适配原生写入协议，默认情况下，写入内核的数据会重新转换为InfluxDB行协议对接后续写入流程
- 设置为true后，将全流程使用原生协议，对接到Shelf写入模式

**rpc-address = "127.0.0.1:8085"**
