...
func ProcessRequest(req msgservice.SysCtrlRequest, resp *bufio.Writer) (err error) {
	...
	case ShardGroupTimeZone:
		...
		return broadcastCmdToStore(req, resp)
	case ObsAkSk:
		if err := CheckUpdateAsSkParamValid(req.Param()); err != nil {
			return err
		}
		return broadcastCmdToStore(req, resp)
	case ObsHostName:
		if err := CheckUpdateStreamConfParamValid(req.Param()); err != nil {
			return err
		}
		return broadcastCmdToStore(req, resp)
	case DBLimit:
		...
		SetDatabaselimit(int32(limit))
		res := "\n\tsuccess"
		WriteString(resp, res)
	case LastRowCache:
		// TODO sendCmdToLocalOrAll
		return broadcastCmdToStore(req, resp)
	case ParquetTask:
