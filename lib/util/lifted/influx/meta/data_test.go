...
func Test_Data_AlterShardsNum(t *testing.T) {
	...
	err = data.CreateShardGroup(dbName, rpName, time.Unix(0, 0).AddDate(0, 0, 2), util.Hot, 0, 0, "")
	if err != nil {
		t.Fatal(err)
	}
	...
	assert2.Equal(t, 1, len(mst.ShardIdexes[sg1.ID]))
	...
	err = data.CreateShardGroup(dbName, rpName, time.Unix(0, 0).AddDate(0, 0, 4), util.Hot, 0, 0, "")
	if err != nil {
		t.Fatal(err)
	}
	...
	assert2.Equal(t, int32(0), mst.InitNumOfShards)
	err = data.CreateShardGroup(dbName, rpName, time.Unix(0, 0).AddDate(0, 0, 6), util.Hot, 0, 0, "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestData_CreateShardGroupZone(t *testing.T) {
	...
	rp.ShardGroups = nil
	rp.IndexGroups = nil
	february := time.Date(2025, 2, 14, 0, 0, 0, 0, time.Local)
