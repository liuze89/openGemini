...
func (h *ColStoreInfo) BuildPropertyMap() {
	h.PropertyMap = make(map[string]string)
	for i := range h.PropertyKey {
		h.PropertyMap[h.PropertyKey[i]] = strings.ToLower(h.PropertyValue[i])
	}
}
...
func (h *ColStoreInfo) UniqueEnabled() bool {
	val, ok := h.PropertyMap[PropertyKeyUniqueEnabled]
	return ok && cast.ToBool(val)
}

func (h *ColStoreInfo) GetPKType() string {
	val, ok := h.PropertyMap[PropertyKeyPKType]
	if !ok {
		val = PrimaryKeyTypeNormal
	}
	return val
}
