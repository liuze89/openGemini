...
func (c *ConditionImpl) convertToRPNElem(rpnExpr *rpn.RPNExpr, opt hybridqp.Options) error {
	for i, expr := range rpnExpr.Val {
		...
		case influxql.Token:
			...
			case influxql.EQ, influxql.LT, influxql.LTE, influxql.GT, influxql.GTE, influxql.NEQ, influxql.MATCHPHRASE, influxql.UNMATCHPHRASE, influxql.MATCH, influxql.LIKE, influxql.IPINRANGE:
			default:
