...
func (r *WriteBlobsRequest) Marshal(buf []byte) ([]byte, error) {
	buf = codec.AppendString(buf, r.Db)
	buf = codec.AppendString(buf, r.Rp)
	buf = codec.AppendUint32(buf, r.Pt)
	buf = codec.AppendUint64(buf, r.Shard)

	buf = codec.AppendUint32(buf, uint32(r.Bg.Size()))
	buf, err := r.Bg.Marshal(buf)
	if err != nil {
		return nil, err
	}
...
func (r *WriteBlobsRequest) Unmarshal(buf []byte) error {
	...
	if r.Bg == nil {
		r.Bg = &shelf.BlobGroup{}
	}
	err := r.Bg.Unmarshal(subBuf)
...
func (r *WriteBlobsRequest) Size() int {
	size := 0
	size += codec.SizeOfString(r.Db)
	size += codec.SizeOfString(r.Rp)
...
func (r *WriteBlobsResponse) Size() int {
	return 1 + 2 + len(r.Message) // the total length of the status code (1 byte), error code (2 bytes), and message content
}
...
func NewSendClearEventsRequest() *SendClearEventsRequest {
	return &SendClearEventsRequest{}
}

func (r *SendClearEventsRequest) Marshal(buf []byte) ([]byte, error) {
	b, err := proto.Marshal(&r.ClearEventsRequest)
	if err != nil {
		return nil, err
	}
...
func (r *SendClearEventsRequest) Unmarshal(buf []byte) error {
	return proto.Unmarshal(buf, &r.ClearEventsRequest)
}
...
func NewSendClearEventsResponse() *SendClearEventsResponse {
	return &SendClearEventsResponse{}
}

func (r *SendClearEventsResponse) Marshal(buf []byte) ([]byte, error) {
	b, err := proto.Marshal(&r.ClearEventsResponse)
	if err != nil {
		return nil, err
	}
...
func (r *SendClearEventsResponse) Unmarshal(buf []byte) error {
	return proto.Unmarshal(buf, &r.ClearEventsResponse)
}
