...
type MockStorage struct {
	...
	onPTLoaded map[uint64]engine.PtLoadFunc
}
...
func (m *MockStorage) RegisterOnPTLoaded(id uint64, f engine.PtLoadFunc) {
	if m.onPTLoaded == nil {
		m.onPTLoaded = make(map[uint64]engine.PtLoadFunc)
	}
	m.onPTLoaded[id] = f
}

func (m *MockStorage) UninstallOnPTLoaded(id uint64) {
	delete(m.onPTLoaded, id)
}
